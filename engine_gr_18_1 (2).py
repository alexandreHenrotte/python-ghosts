...
def read_file(file):
    ...
    return {
        "number_magic": number_magic,
        "board_dictionnary": board_dictionnary,
        "player_1": player_1,
        "player_2": player_2
    }


def set_grid_ghost(x, y, player, item, normal_bg_color, critical_bg_color):
    for p in range(len(player["ghost"])):
        if player['ghost'][p]['number_magic'] > 10:
            print(normal_bg_color +
                  term.move_xy(x*3-1, y+1) + item)
        else:
            print(critical_bg_color +
                  term.move_xy(x*3-1, y+1) + item)
...
def board(player_1, player_2, number_magic, board_dictionnary):
    ...
    for n in range(1, 21):
        if n <= 9:
            ...
            print(" " + str(n), end='')
    print()
    for x in range(1, board_dictionnary["hauteur"]+1):
        # numbers on the side
        if x <= 9:
            print(' ' + str(x), end='')
        else:
        ...
        for y in range(1, board_dictionnary["largeur"]+1):
    ...
    for e in number_magic:
        for a in range(len(number_magic[e])):
            ...
            if e == 10:
                print(term.red_on_yellow +
                      term.move_xy(magic_bag[0]*3-1, magic_bag[1]+1) + ' 10')
            elif e == 30:
    ...
    # magic of each player on the side of the board
    print(term.normal + term.move_xy(21*3, 1) + term.darkblue +
          'magic player 1: ' + str(player_1['number_magic']))
    print(term.normal + term.move_xy(21*3, 2) + term.darkred +
          'magic player 2: ' + str(player_2['number_magic']))
...
def create_ghost(player):
    ...
    if check_ghost == False:
        if player["number_magic"] >= 300:
            ...
            set_grid_ghost(
                position[0], position[1], player, GHOST_PLAYER, term.on_red, term.on_darkred)
            return player['ghost']
        else:
            print(
                'you do not have enough money and your ghost dictionary is staying %s' % player)
    else:
...
def is_there_a_ghost(player, position):
    ...
    for element in range(len(player['ghost'])):
        print(player['ghost'][element].keys())
    ...
    if player['number_magic'] < magic*2:
        ...
        if index == -99:
            ...
            if player['ghost'][index]['number_magic'] == 100:
                ...
                print(
                    'you cannot add that much magic to your ghost, your maximum is %s.' % maximum)
            else:
        ...
        for element in range(len(player_2['ghost'])):
            ...
            for x in number_magic:
                for element in number_magic[x]:
                    ...
                    move['ghost'].append(
                        {'initial_position': [row, column], 'distance': ghost_distance})
                    ...
                    magic_distance = get_distance(
                        a, b, row_magic, column_magic)
                    move['magic'].append(
                        {'initial_position': [row_magic, column_magic], 'distance': magic_distance})
...
def move_ghost_AI(position_to_go, position_ghost_AI, player, number_magic):
...
def get_AI_orders(player_1, player_2, number_magic, board_dictionnary, player_id):
    """Return orders of AI.
    ...
    if player_id == 1:
        for e in range(len(player_1['ghost'])):
            ...
            positions = disponible_place(
                player_1, position_ghost_AI, board_dictionnary)
            position_to_go = positions[0]
            orders = move_ghost_AI(
                position_to_go, position_ghost_AI, player_1, number_magic)
    elif player_id == 2:
        for e in range(len(player_1['ghost'])):
            position_ghost_AI = player_2['ghost'][e]['position']
            positions = disponible_place(
                player_2, position_ghost_AI, board_dictionnary)
            position_to_go = positions[0]
            orders = move_ghost_AI(
                position_to_go, position_ghost_AI, player_2, number_magic)
    return orders

# main function


def play_game(map, type_1, type_2):
    """Play a game.
    ...
    number_magic = map["number_magic"]
    board_dictionnary = map["board_dictionnary"]
    player_1 = map["player_1"]
    player_2 = map["player_2"]
    print(player_1)
    group_1 = player_1["group"]
    group_2 = player_2["group"]
    ...
    # ajouter le plateau
    board(player_1, player_2, number_magic, board_dictionnary)
    ...
    while turn <= 10:
        ...
        # get orders of player 2 if human and notify them to player 1, if necessary
            ...
            if type_1 == 'remote':
                notify_remote_orders(connection, orders)
            read_order(order_2, player_2)

        # get orders of player 1 and notify them to player 2, if necessary
        if type_1 == 'remote':
            orders = get_remote_orders(connection)
        else:
            orders = get_AI_orders(
                player_1, player_2, number_magic, board_dictionnary, 1)
            if type_2 == 'remote':
                notify_remote_orders(connection, orders)
        read_order(orders, player_1)
...
def create_server_socket(local_port, verbose):
    ...
    # deal with a socket in TIME_WAIT state
    ...
    try:
        ...
        raise IOError(
            'local port %d already in use by your group or the referee' % local_port)
    socket_in.listen(1)

    if verbose:
        print(
            '   done -> can now accept a remote connection on local port %d\n' % local_port)
...
def create_client_socket(remote_IP, remote_port, verbose):
    ...
    # deal with a socket in TIME_WAIT state
    socket_out.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
    ...
    while not connected:
        try:
            if verbose and not msg_shown:
                print(' connecting on %s:%d to send orders' %
